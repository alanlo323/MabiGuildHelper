...
namespace DiscordBot.Extension
{
    public static class SemanticKernelKExtension
    {
        ...
        {
            return kernelArguments == null || kernelArguments.Count == 0 ? string.Empty : kernelArguments.Select(x => $"{x.Value}").Aggregate((s1, s2) => $"{s1}-{s2}");
        }

        public static string ToDisplayName(this KernelArguments kernelArguments)
        {
            if (kernelArguments == null) return string.Empty;
            string argumentsStr = kernelArguments.Select(x => $"{x.Value}").Aggregate((s1, s2) => $"{s1} {s2}");
            return kernelArguments == null ? string.Empty : $"  [{argumentsStr}]";
        }
    }
}
